...
REM ============================================================================
REM  Podprogrammy (CMD trebuet metki do osnovnogo koda ili posle goto)
REM ============================================================================
...
call :AppendLog "Attempt !ATTEMPT!/!MAX_ATTEMPTS! key=!OUT_KEY!"
...
if not errorlevel 1 (
    ...
    call :AppendLog "WARN: output missing for !OUT_KEY!"
)
...
if not defined WORK_SRC (
    ...
    call :AppendLog "[ERROR] copy failed"
    exit /b 0
)
...
if !TOTAL! equ 0 (
    ...
    call :PrintLine "Proverite: %INPUT_DIR%"
)

call :PrintLine "Log file:"
call :PrintLine "!LOG_FILE!"
call :AppendLog "Finished. Total=!TOTAL!"
