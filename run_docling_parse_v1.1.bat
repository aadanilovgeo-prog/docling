...
if /i "%CLI_EXT%"==".pptx" set "IS_OFFICE=1" & set "FROM_FLAG=--from pptx"
if /i "%CLI_EXT%"==".docx" set "IS_OFFICE=1" & set "FROM_FLAG=--from docx"
if /i "%CLI_EXT%"==".xlsx" set "IS_OFFICE=1" & set "FROM_FLAG=--from xlsx"

:DoclingAttempt
set /a ATTEMPT+=1
call :CleanupOutputArtifacts "!OUT_KEY!"
call :AppendLog "Attempt !ATTEMPT!/!MAX_ATTEMPTS! ext=!CLI_EXT! key=!OUT_KEY!"

REM Office (pptx/docx/xlsx): bez OCR s pervoj popytki
set "OCR_FLAG=--ocr"
if "!IS_OFFICE!"=="1" set "OCR_FLAG=--no-ocr"
if !ATTEMPT! geq 2 set "OCR_FLAG=--no-ocr"

if "!USE_PYPDF!"=="1" (
    docling --to md --to html --output "%OUTPUT_DIR%" !FROM_FLAG! !OCR_FLAG! --tables --table-mode accurate --image-export-mode placeholder --pdf-backend pypdfium2 -v "!CLI_SRC!" >> "!LOG_FILE!" 2>&1
) else (
    docling --to md --to html --output "%OUTPUT_DIR%" !FROM_FLAG! !OCR_FLAG! --tables --table-mode accurate --image-export-mode placeholder -v "!CLI_SRC!" >> "!LOG_FILE!" 2>&1
)
...
if not errorlevel 1 (
    ...
    call :AppendLog "WARN: md/html missing for !OUT_KEY!"
)
...
if not defined WORK_SRC (
    ...
    call :AppendLog "[ERROR] copy failed - zakroyte fajl v Office"
    exit /b 0
)
...
if "!PARSE_FAILED!"=="1" (
    ...
    call :AppendLog "[PARSE] done md+html"
    call :RemoveUnwantedOutputs "!OUT_KEY!"
)
exit /b 0
...
call :PrintLine "Output formats: md, html only"
...
call :AppendLog "Started v!SCRIPT_VERSION! md+html only"
