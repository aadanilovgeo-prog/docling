#!/usr/bin/env python3
"""
Docling batch parser v2.0.8 — Python port of BAT v1.6 logic.
...
def open_image_size(path: Path) -> tuple[int, int] | None:
    """PIL dimensions — same source used for tile split."""
    apply_pillow_limit()
    try:
        from PIL import Image
...
def split_image_vertical(
    ...
    with Image.open(src) as img:
        ...
        while y < h:
            ...
            y = y_end
            idx += 1
    expected = tiles_for_vertical(h, tile_height)
    log_append(
        app,
        f"OCR split vertical: {src.name} {w}x{h} -> {len(tiles)} tiles (h<={tile_height})",
    )
    if len(tiles) != expected:
        log_append(app, f"WARN: vertical tile count {len(tiles)} != expected {expected}")
    return tiles, temps
...
def split_image_horizontal(
    ...
    with Image.open(src) as img:
        ...
        while x < w:
            ...
            x = x_end
            idx += 1
    expected = tiles_for_horizontal(w, tile_width)
    log_append(
        app,
        f"OCR split horizontal: {src.name} {w}x{h} -> {len(tiles)} tiles (w<={tile_width})",
    )
    if len(tiles) != expected:
        log_append(app, f"WARN: horizontal tile count {len(tiles)} != expected {expected}")
    return tiles, temps
...
def prepare_ocr_inputs(
    ...
    attempt: int,
    max_side: int,
) -> tuple[list[Path], list[Path], TilePlan]:
    """Prepare one or more images for OCR (split scroll captures, else downscale)."""
    size = open_image_size(src)
    if not size:
        return [src], [], TilePlan(1, "single", "unknown size")
    w, h = size
    plan = plan_ocr_tiles(w, h, max_side)
    if plan.count == 1 and plan.mode == "single":
        return [src], [], plan

    if plan.mode == "vertical":
        tiles, temps = split_image_vertical(app, src, job_id, attempt, max_side)
    elif plan.mode == "horizontal":
        tiles, temps = split_image_horizontal(app, src, job_id, attempt, max_side)
    else:
        dest, temp = downscale_uniform(app, src, job_id, attempt, max_side)
        return [dest], [temp], plan
...
def run_with_retry(app: App, src: Path, job_id: str, out_key: str, ext: str) -> bool:
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        if spec.kind == "image" and use_ocr:
            ...
            ocr_inputs, ocr_temps, tile_plan = prepare_ocr_inputs(
                app, src, job_id, attempt, max_side,
            )
...
def main() -> int:
    ...
    if runtime is None:
        ...
        say("  python run_docling_parse_v2.0.8.py --python C:\\...\\miniconda3\\python.exe")
