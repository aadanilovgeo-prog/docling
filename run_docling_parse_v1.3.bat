...
if /i "!FMT_KIND!"=="unknown" (
    call :AppendLog "ERROR: nepodderzhivaemoe rasshirenie .!EXT_NORM!"
    exit /b 1
)
...
call :AppendLog "Attempt !ATTEMPT!/!MAX_ATTEMPTS! kind=!FMT_KIND! file=!OUT_KEY!"
call :PrintLine "  Attempt !ATTEMPT!/!MAX_ATTEMPTS! [!FMT_KIND!] !OUT_KEY!"
...
if not errorlevel 1 (
    ...
    call :AppendLog "WARN: net fajlov md/html"
)

if !ATTEMPT! lss !MAX_ATTEMPTS! (
    call :AppendLog "Povtor cherez !RETRY_DELAY_SEC! sek..."
    call :PrintLine "  Povtor cherez !RETRY_DELAY_SEC! sek..."
...
call :PrintLine "Vyhod: md + html (bez artefaktov)"
call :PrintLine "========================================"
...
if not exist "%LOG_FILE%" set "LOG_FILE=%ROOT_DIR%\docling_%LOG_STAMP%.log"
...
if errorlevel 1 (
    call :PrintLine ""
    call :PrintLine "OSHIBKA: Docling ne najden v PATH."
    call :PrintLine "Ustanovite: pip install docling"
...
if not exist "%INPUT_DIR%" (
    call :PrintLine ""
    call :PrintLine "OSHIBKA: Papka docs ne najdena:"
...
call :PrintLine ""
call :PrintLine "Skanirovanie fajlov..."
call :PrintLine ""
...
if !TOTAL! equ 0 (
    call :PrintLine ""
    call :PrintLine "VNIMANIE: Podderzhivaemye fajly ne najdeny."
    call :PrintLine "Polozhite dokumenty v: %INPUT_DIR%"
)
...
echo.
echo ========================================
echo Nazhmite lyubuyu klavishu dlya zakrytiya...
echo ========================================
pause >nul
endlocal
exit /b 0
