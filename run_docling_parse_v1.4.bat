...
REM Zapusk cherez cmd /k - okno ne zakroetsya pri oshibke ili dvojnom klke
if /i not "%~1"=="RUN" (
    title Docling batch v1.4
    cmd /k call "%~f0" RUN
    exit /b 0
)
...
call :AppendLog "Attempt !ATTEMPT!/!MAX_ATTEMPTS! !FMT_KIND! !OUT_KEY!"
...
if not defined WORK_SRC (
    ...
    call :AppendLog "[ERROR] copy fail"
    exit /b 0
)
...
if errorlevel 1 (
    ...
    call :AppendLog "ERROR docling PATH"
    goto :EndScript
)
...
:EndScript
call :PrintLine ""
call :PrintLine "Gotovo. Okno cmd /k - zakroyte vruchnuyu ili vvjdite exit"
exit /b 0
